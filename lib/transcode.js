...
function transcode(input, output, preset, cb){
  ...
  inputFormat.decode([inputAudio, inputVideo], function(stream, frame, progress){
    counter++;
    if(frame){
      if(counter%100 === 0){
        cb && cb(progress);
      }
      if(stream.codec.codec_type === outputVideo.codec.codec_type){
        frame = converter.convert(frame);
        outputFormat.encode(outputVideo, frame);
      }
      
      if(stream.codec.codec_type === outputAudio.codec.codec_type){
